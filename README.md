...
| 3.4 | [3_4_dynamic_tools.ipynb](module_3/3_4_dynamic_tools.ipynb) | Restricting which tools are available to the model per-request with `@wrap_model_call`, based on runtime context (e.g. giving "internal" users access to a SQL tool but "external" users only web search) |
