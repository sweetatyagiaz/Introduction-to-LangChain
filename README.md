...
| 3.5 | [3_5_email_agent.ipynb](notebooks/module-3/3.5_email_agent.ipynb) / [.py](notebooks/module-3/3.5_email_agent.py) | Capstone: an email agent that combines everything from Module 3 — an `authenticate` tool that writes to custom state, dynamic tools/prompt middleware that only unlock the inbox once authenticated, and `HumanInTheLoopMiddleware` requiring approval before any email is actually sent |
